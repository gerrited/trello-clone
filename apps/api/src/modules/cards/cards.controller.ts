...
export async function createHandler(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const card = await cardsService.createCard(req.params.boardId as string, req.userId!, req.body);
...
export async function deleteHandler(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    await cardsService.deleteCard(req.params.cardId as string, req.userId!);
    res.status(204).end();
  } catch (err) {
    next(err);
  }
